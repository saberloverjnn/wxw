...
int main(int argc, char* argv[])
{
    ...
    //设置传输对端套接字地址
    struct sockaddr_in serveraddr;     //define structural variable type
    ...
    serveraddr.sin_port = htons(SERVERPORT);  //将一个无符号短整型数值转换为网络字节序(大端模式)
    ...
    //建立连接
    if (connect(sockfd, (const struct sockaddr *) &serveraddr, sizeof(serveraddr)) < 0)  //用于创建与指定外部端口的连接 s:标识一个未连接socket  serveraddr:指向要连接套接字的sockaddr结构体的指针
    {
